...
BICScore::BICScore(BayesianNetwork _model, DataSet _dataset): model(_model), dataset(_dataset){

}

BICScore::BICScore(DataSet _dataset): dataset(_dataset){

}

double BICScore::get_score(){
  double score = 0;
...
double BICScore::get_local_score(Node variable, vector<Node>& parents){
  ...
  for(auto pitr = statecount.countmap.begin(); pitr != statecount.countmap.end(); pitr++){
    ...
    for(auto citr = childmap.begin(); citr != childmap.end(); citr++){
      ...
      if(nijk > 0) local_score += nijk * (log(nijk) - log(nik));
    }
  }
  local_score -= 0.5 * log(this->dataset.data.size()) * (double)statecount.parents_candidates_num * ((double)statecount.child_candidates_num - 1);
  return local_score;
}

BICScore::StateCount BICScore::get_state_counts(Node variable, vector<Node>& parents){
  ...
  if(parents.size() > 0){
    ...
    return BICScore::StateCount(parents_candidates_num, child_candidates_num, family_states_countmap);
    ...
    return BICScore::StateCount(1, child_candidates_num, states_countmap);
  }
}
